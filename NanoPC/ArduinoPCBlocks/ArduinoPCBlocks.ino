...
{
  os::fill();
  os::text(F("A R D U I N O   B L O C K S"), 6, 10);
  os::text(F("Press  SPACE"), 13, 23);
}

void drawfield()
{
  os::fill();
  for (byte i=0; i<5; i++) os::text(F("<!..........!>"), 12, i);
  for (byte i=5; i<20; i++) os::text(F("<!..........!>"), 12, i);
  os::text(F("<!==========!>"), 12, 20);
  os::text(F("  VVVVVVVVVV  "), 12, 21);
  os::text(F("SCORE 0"), 0, 0);
  os::text(F("HIGH"), 27, 0);    
  os::text(String(high, DEC), 32, 0);
  os::text(F("CONTROLS"), 0, 3);
  os::text(F("A - Left"), 0, 5);
  os::text(F("D - Right"), 0, 6);
  os::text(F("W - Rotate"), 0, 7);
  os::text(F("S - Drop"), 0, 8);
}
